...
def _empirical_reliability() -> dict[str, Any]:
    ...
    if not overall or int(overall[0] or 0) == 0:
        return {"available": False, "reason": "No resolved outcomes for active model", "model_version": main.MODEL_VERSION}

    quality = [_metric_row(r) for r in quality_rows]
    sigma = [_metric_row(r) for r in sigma_rows]
    cohorts = []
    total_current = int(overall[0])
    for r in cohort_rows:
        pn = int(r[8]); ps = int(r[9] or 0); n = int(r[1])
        cohorts.append({"cohort": r[0], "n": n, "share": round(n / total_current, 6) if total_current else None,"accuracy_1x2": round(float(r[3]), 6) if r[3] is not None else None,"accuracy_1x2_ci95": _wilson_interval(int(r[2]), n),"brier_1x2": round(float(r[4]), 6) if r[4] is not None else None,"logloss_1x2": round(float(r[5]), 6) if r[5] is not None else None,"legacy_primary_n": pn,"legacy_primary_accuracy": round(float(r[10]), 6) if r[10] is not None else None,"legacy_primary_accuracy_ci95": _wilson_interval(ps, pn) if pn else None,"avg_sigma": round(float(r[11]), 6) if r[11] is not None else None})
    ...
    n = int(overall[0]); successes = int(overall[1] or 0)
    brier = round(float(overall[3]), 6) if overall[3] is not None else None
    logloss = round(float(overall[4]), 6) if overall[4] is not None else None
