...
app = FastAPI(title="S2S Sigma Production Engine - Always Active")
...
async def get_props():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            # Si no hay encuentros para hoy, consultamos los de mañana como respaldo
            if not fixtures:
                fecha_manana = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")
                url_manana = f"{BASE_URL}/fixtures?date={fecha_manana}"
                resp_m = await client.get(url_manana, headers=HEADERS)
                if resp_m.status_code == 200:
                    fixtures = resp_m.json().get("response", [])
            ...
            for idx, fix in enumerate(fixtures[:30]):
                ...
                partidos_consolidados.append({
                    ...
                    "contexto_defensa": f"{away_name} cede fuera 1.2 goles/juego",
