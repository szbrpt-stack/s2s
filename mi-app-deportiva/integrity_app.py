...
def _empirical_reliability() -> dict[str, Any]:
    ...
    # All headline reliability metrics are now restricted to the active model
    # version. Historical model versions remain available only as diagnostics.
    ...
    with main.psycopg.connect(main.DATABASE_URL, connect_timeout=15, prepare_threshold=None) as db:
        quality_rows = db.execute(quality_sql, params).fetchall()
        sigma_rows = db.execute(sigma_sql, params).fetchall()
        overall = db.execute(overall_sql, params).fetchone()
        cohort_rows = db.execute(cohort_sql, params).fetchall()
        windows = db.execute(windows_sql, params).fetchone()
        version_rows = db.execute(versions_sql).fetchall()

    quality = [_metric_row(r) for r in quality_rows]
    sigma = [_metric_row(r) for r in sigma_rows]
    cohorts = []
    total_current = int(overall[0]) if overall else 0
    for r in cohort_rows:
        pn = int(r[8]); ps = int(r[9] or 0); n = int(r[1])
        cohorts.append({
            "cohort": r[0], "n": n,
            "share": round(n / total_current, 6) if total_current else None,
            "accuracy_1x2": round(float(r[3]), 6) if r[3] is not None else None,
            "accuracy_1x2_ci95": _wilson_interval(int(r[2]), n),
            "brier_1x2": round(float(r[4]), 6) if r[4] is not None else None,
            "logloss_1x2": round(float(r[5]), 6) if r[5] is not None else None,
            "legacy_primary_n": pn,
            "legacy_primary_accuracy": round(float(r[10]), 6) if r[10] is not None else None,
            "legacy_primary_accuracy_ci95": _wilson_interval(ps, pn) if pn else None,
            "avg_sigma": round(float(r[11]), 6) if r[11] is not None else None,
        })
    ...
    n = int(overall[0]); successes = int(overall[1]); brier = round(float(overall[3]), 6); logloss = round(float(overall[4]), 6)
    bb = round(float(overall[7]), 6) if overall[7] is not None else None
    bl = round(float(overall[8]), 6) if overall[8] is not None else None
    w24n, w24acc, w7n, w7acc = windows

    return {
        "available": True,
        "model_version": main.MODEL_VERSION,
        "headline": {
            "metric_name": "accuracy_1x2_model",
            "n": n,
            "accuracy_1x2_model": round(float(overall[2]), 6),
            "accuracy_1x2_ci95": _wilson_interval(successes, n),
            "window_24h": {"n": int(w24n or 0), "accuracy_1x2_model": round(float(w24acc), 6) if w24acc is not None else None},
            "window_7d": {"n": int(w7n or 0), "accuracy_1x2_model": round(float(w7acc), 6) if w7acc is not None else None},
            "definition": "Exactitud del resultado 1X2 del modelo activo; no mezcla versiones ni cohortes heredadas.",
        },
        "probabilistic_quality": {
            "brier_1x2": brier,
            "logloss_1x2": logloss,
            "brier_over25": round(float(overall[5]), 6) if overall[5] is not None else None,
            "brier_btts": round(float(overall[6]), 6) if overall[6] is not None else None,
            "baseline_brier_1x2": bb,
            "baseline_logloss_1x2": bl,
            "beats_baseline_brier_1x2": bb is not None and brier < bb,
            "beats_baseline_logloss_1x2": bl is not None and logloss < bl,
        },
        "legacy_cohorts": cohorts,
        "historical_versions": [
            {"model_version": r[0], "n": int(r[1]), "accuracy_1x2": round(float(r[2]), 6) if r[2] is not None else None, "brier_1x2": round(float(r[3]), 6) if r[3] is not None else None}
            for r in version_rows
        ],
        "by_data_quality": quality,
        "by_sigma": sigma,
        "diagnostics": {
            "data_quality_monotonic_accuracy": mono_acc,
            "data_quality_monotonic_brier": mono_brier,
            "data_quality_is_empirical_confidence": mono_acc and mono_brier,
            "interpretation": "evidence_quality mide procedencia/completitud y permanece separada de la fiabilidad empírica.",
        },
        "reporting_policy": {
            "generic_winrate_field_removed": True,
            "headline_uses_active_model_only": True,
            "historical_versions_are_diagnostic_only": True,
            "legacy_primary_metrics_are_not_headline_metrics": True,
